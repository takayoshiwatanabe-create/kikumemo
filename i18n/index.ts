...
// These exports are client-side only due to `getDeviceLanguage()`.
// For server-side, the locale should be derived from the URL.
// Removed `lang`, `isRTL`, `translate` as direct exports to avoid confusion with `useI18n` and SSR issues.
// If needed for non-React context, a separate server-safe utility would be better.
